/**
 * Address book — named recipients saved on this device (via the storage shim →
 * AsyncStorage). Lets the user send to "alice" instead of a 42-char hex string: the Send
 * panel resolves a saved name to its address (client-side, like ENS). Only public data
 * (name + address) is stored; nothing here touches keys. Addresses are validated on add so
 * a typo can't be saved. (Ported from apps/web/src/contacts.ts.)
 */
...
// bech32 is case-insensitive (an all-uppercase BC1…/TB1… is a valid encoding); accept either case.
// addContact normalizes a matched BTC address to canonical lowercase before storing.
const BTC = /^(bc1|tb1)[0-9a-z]{20,80}$/iu;
...
function read(): Contact[] {
  try {
    ...
    // Validate FIELD TYPES, not just object shape: a malformed element ({} or {name:123}) from a
    // legacy/tampered store parses fine but then crashes consumers doing name.localeCompare / address.trim
    // / toLowerCase on the chat-plan, send-preview, and knownGoodAddresses poison-guard paths (a single
    // bad row would otherwise throw and DISABLE the poison check before a broadcast). Drop bad entries —
    // parity with web contacts.ts.
...
function write(contacts: Contact[]): void {
  storage.setItem(KEY, JSON.stringify(contacts));
}
...
export function addContact(name: string, address: string): Contact[] {
  ...
  // Canonicalize bech32 to lowercase (BC1… and bc1… are the same address); leave EVM checksummed.
  const stored = kind === 'btc' ? a.toLowerCase() : a;
  const contacts = read();
  if (contacts.some((c) => c.name.toLowerCase() === n.toLowerCase())) throw new Error('A contact with that name already exists.');
  if (contacts.some((c) => c.address.toLowerCase() === stored.toLowerCase())) throw new Error('That address is already saved.');
  contacts.push({ name: n, address: stored, kind });
  write(contacts);
  return listContacts();
}
...
// Contact names that collide with an asset ticker/name, a command/connector word, or a pure number are
// NOT substituted: whole-word replacing them corrupts the amount/asset/command region of an utterance (a
// contact named "SOL" turns "send 2 SOL to alice" into "send 2 <0x…> to alice", destroying the asset — and
// the injected address can then be misread as the recipient). Kept IN SYNC with web apps/web/src/contacts.ts
// and the parser registry (KNOWN_ASSETS + NAME_ALIASES in @intent-wallet/intents); the mobile port had
// dropped this guard, so mobile chat was corruptible where web was safe. Recipient-name slot is unaffected.
